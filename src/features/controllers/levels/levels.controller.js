import { CreateLevelSchema, UpdateLevelSchema } from "@/features/services/levels/levels.schema";
import { LevelService } from "@/features/services/levels/levels.service";
import { levelRepository } from "@/features/repositories/levels/levels.repository";
export var LevelController;
(function (LevelController) {
    LevelController.getAllLevelsHandler = async (req, res, next) => {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 10;
            const result = await LevelService.getAllLevels(page, limit);
            res
                .status(200)
                .json({
                message: "Levels retrieved successfully",
                data: result.levels,
                meta: {
                    total: result.total,
                    page: result.page,
                    limit: result.limit,
                    totalPages: result.totalPages
                }
            });
        }
        catch (error) {
            next(error);
        }
    };
    LevelController.getLevelByIdHandler = async (req, res, next) => {
        try {
            const id = parseInt(req.params.id);
            if (isNaN(id)) {
                return res.status(400).json({ message: "Invalid level ID" });
            }
            const level = await LevelService.getLevelById(id);
            if (!level) {
                return res.status(404).json({ message: "Level not found" });
            }
            res.status(200).json({ message: "Level retrieved successfully", data: level });
        }
        catch (error) {
            next(error);
        }
    };
    LevelController.getLevelByNameHandler = async (req, res, next) => {
        try {
            const name = req.query.name;
            if (!name) {
                return res.status(400).json({ message: "Level name is required" });
            }
            const level = await levelRepository.getLevelByName(name);
            if (!level) {
                return res.status(404).json({ message: "Level not found" });
            }
            res.status(200).json({ message: "Level retrieved successfully", data: level });
        }
        catch (error) {
            next(error);
        }
    };
    LevelController.createLevelHandler = async (req, res, next) => {
        try {
            const parsedData = CreateLevelSchema.parse(req.body);
            const level = await LevelService.createLevel(parsedData);
            res
                .status(201)
                .json({ message: "Level created successfully", data: level });
        }
        catch (error) {
            next(error);
        }
    };
    LevelController.updateLevelHandler = async (req, res, next) => {
        try {
            const id = parseInt(req.params.id);
            if (isNaN(id)) {
                return res.status(400).json({ message: "Invalid level ID" });
            }
            const parsedData = UpdateLevelSchema.parse(req.body);
            const level = await LevelService.updateLevel(id, parsedData);
            res
                .status(200)
                .json({ message: "Level updated successfully", data: level });
        }
        catch (error) {
            next(error);
        }
    };
    LevelController.deleteLevelHandler = async (req, res, next) => {
        try {
            const id = parseInt(req.params.id);
            if (isNaN(id)) {
                return res.status(400).json({ message: "Invalid level ID" });
            }
            const level = await LevelService.deleteLevel(id);
            res
                .status(200)
                .json({ message: "Level deleted successfully", data: level });
        }
        catch (error) {
            next(error);
        }
    };
})(LevelController || (LevelController = {}));
