...
export var EvaluationController;
(function (EvaluationController) {
    EvaluationController.getAllEvaluationsHandler = async (req, res) => {
        try {
    ...
    EvaluationController.getEvaluationByIdHandler = async (req, res) => {
        try {
    ...
    EvaluationController.createEvaluationHandler = async (req, res) => {
        try {
    ...
    EvaluationController.updateEvaluationHandler = async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            if (isNaN(id)) {
                return res.status(400).json({ message: "Invalid evaluation ID" });
    ...
    EvaluationController.deleteEvaluationHandler = async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            if (isNaN(id)) {
                return res.status(400).json({ message: "Invalid evaluation ID" });
